# Cover artwork — usable-compute

![An open compute chassis revealing the network, copper power bus and cooling support beneath two modules.](social-preview-v1.jpg)
...
Open [repository settings](https://github.com/dimaggi-ai/usable-compute/settings),
...
Alt text: An open compute chassis revealing the network, copper power bus and cooling support beneath two modules.
...
Subject: usable compute emerges from several physical infrastructure layers working together. A single refined architectural cutaway of a small compute pod: two low finned graphite compute modules, one copper power busbar underneath, one pale-metal coolant rail alongside, and one short clean network connection joining the modules. The parts are integrated into one shallow open chassis, with one corner left open to reveal the supporting layers. A compact, coherent whole, five main forms at most. No collection of separate icons, no diagrams, no charts, no balance scales, no skyline. Let the tangible interdependence be clear through the cutaway.
```
